    ...
    onLoad() {
        unpatch = cumcord.patcher.after("sendMessage", webpackModules.findByProps("sendMessage"), (args) => {
            if (args[1].content.startsWith("!cid")) {
                args[1].content = webpackModules.findByProps("getChannelId").getChannelId();
            } else if (args[1].content.startsWith("!uid")) {
            	try {
      				var new_msg = args[1].content.slice(args[1].content.indexOf("<@!")+3).replace(">", "").substring(0, 19); //big long line to isolate the uid
